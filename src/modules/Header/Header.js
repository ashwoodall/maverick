    ...
    <AppBar className={ theme.header } flat fixed>
      <Flexbox className={ theme.wrapper } layout='row' flex align='space-between center'>
        ...
        <Navigation className={ theme.navigation }>
          <Link className={ active === null || active === '/people' ? classnames(theme.link, theme.active) : theme.link } label='People' icon='people' onClick={ () => handleClick('people') } />
          <Link className={ active === '/messages' ? classnames(theme.link, theme.active) : theme.link } label='Messages' icon='forum' onClick={ () => handleClick('messages') } />
          <Link className={ active === '/profile' ? classnames(theme.link, theme.active) : theme.link } label='Profile' onClick={ () => handleClick('profile') } icon={ <Avatar className={ theme.avatar } image='https://scontent-dft4-2.xx.fbcdn.net/v/t1.0-1/p320x320/14925568_351751435169655_5558516257312023873_n.jpg?oh=22d89b90395390c2764b6d81462a1ae7&oe=58BD890C' /> } />
        </Navigation>
      </Flexbox>
    </AppBar>
  </div>
